...
- `cwd` (REQUIRED for `self-managing`; N/A for `worker` since its trio drops the CWD-bearing modules) — this agent's **Workspace (cwd)**, **path ending in `/`**. Each agent has its own; Fills `{{CWD}}` (so `{{CWD}}persona/`, `{{CWD}}AGENT_LOG.md` resolve correctly). Ask if a self-managing agent's cwd is not given.
...
(en agents: same line in English.) Write in the file's language; don't bake host/port into the trio.
...
2. To deploy: open Lobster App UI for agent `<agent_id>`, paste IDENTITY/SOUL/USER into the three boxes (the zh trio by default; the en trio only if you generated bilingual), save (UI dual-writes sqlite+Agent Core), then start a new session to verify.
3. `TOOLS.md` / `MEMORY.md` are runtime files, NOT UI boxes — copy them into Agent Core once (or let the agent author them on first run). The proxy line and the volatile current-status line live here, not in the trio.
4. Keep this `<agent_id>/` folder as the agent's Workspace (cwd) persona canonical; put it under local git.
5. Reminder: never edit the live Agent Core trio directly — always edit canonical here, then re-deploy via UI.
