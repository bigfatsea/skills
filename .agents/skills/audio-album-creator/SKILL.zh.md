...
1. **Suno 抽卡区（逐首一块，复制即用）**：每首一个分隔块——标题 + 定位（主打/副主打/底座/间奏）+ 目标分；`Style`（4–7 描述符、人声三层在前、含调性）+ `Exclude` + `Sliders` 整行可复制；完整歌词（英文 metatag、副歌写全）整块可粘贴进 Suno Lyrics；本曲抽卡 SOP（N 值、Inpainting 次数、重点听什么——主打盯锚点句倒字 / 副主打盯旋律记忆点 / 底座合格即收）。器乐间奏单列（开 Instrumental、Lyrics 留空、给 Style + 结构说明）。**全碟 Sliders 记法须统一成一种**（定性 Safe/Strong 或百分比二选一，不得各曲不一），消除散乱。
