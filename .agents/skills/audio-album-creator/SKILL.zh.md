...
提炼后**写一份中间文件** `material-distillation-{subject}.md` 存入工作目录，含：
...
- 风格家族【默认】（某轨可按节点覆盖）：{风格家族} + {核心配器} + {制作}——某轨可换流派/时代色配合其节点；大幅多样化→靠 Persona 而非 Custom Model（§24）
...
- 本曲【流派/时代色】槽——由其叙事节点驱动（贴合该节点的声音：其情感，及故事所带的任何年代/地方/事件/文化联想；框架 §15.1/§34）；除非节点要求切换，否则默认沿用风格家族
...
- **输入** = 创作契约卡（全专宪法）+ 本曲档案（定位/锚点/杠杆/throughline 触点）+ **`references/lyric-craft.zh.md`**（逐曲词作工具卡：七维度 / 三轮打磨 / 主打锤炼）+ `references/album-creation-guide.zh.md` 路径备查。
- **任务** = 产出 ①3 个候选歌名；②若干版完整中文歌词（每版过 `references/lyric-craft.zh.md` 的"三轮打磨协议"）；③Suno Style 曲风；④自评择优选出推荐版，附简短择优理由 + 倒字/QC 简注。
