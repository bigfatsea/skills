...
# Album Creation Guide · 专辑创作指南 v4.0（精简版）
...
> **曲目量是从素材里诊断出的"节拍预算"，不是先定格数再往里塞**：6–8 首是首推默认区间(短故事/单一切面偏 6，长故事/多事件偏 8)；当素材真实的节拍量明显要求时，5 首或 9–10 首也可接受，但再往外漂需要有理由。比数字更重要的是**弧光的形状**——须先从素材里诊断出来，再决定曲目量与曲线，它是一个工具箱，不是清单，以下都不是默认模板：
> - **经典弧光**：铺陈→冲突上升→高潮→回落→结局。适合有清晰外部冲突或转折点的素材。
> - **螺旋 / 内省式弧光**：每首都在从不同侧面回望同一个核心创伤——向内深掘而非向前推进，深度优先于进展。
> - **渐进无高潮式弧光**：平淡但持续累积的一生——没有激励事件、没有外部冲突、没有可供戏剧化的转折点。如果真有"峰值"，那也只是情绪浓度或暖意最盛之处，而非情节事件。硬给这种形状按一个高潮，会造出一个虚假、空心的顶点。
> - **碎片 / 诗性拼贴式弧光**：有意的留白与非线性跳切，邀请听者自行拼凑。
> 若有某种公认的叙事理论(三幕剧、Freytag 金字塔、起承转合……)确实契合这份素材，欢迎借用——那也只是工具箱里的又一件工具，绝非必套的模板。**绝不能给素材本没有的高潮、激励事件或转折点硬造一个出来。**
...
**拍号情感**：4/4笃定·3/4回旋·6/8摇篮·2/4明快。**单曲时长甜区2:30–3:30**；总时长随曲目数浮动(专辑8首约26–30分，6首约20–22分)。
...
## 34. 专辑统筹
**一致性来自不变量 binder(Persona+主题动机+统一调性)，而非单一统一流派**(§24)——所以流派/时代色可随叙事节点逐曲变化。主题动机：代表作旋律/一句歌词在别曲变奏或器乐复现；题眼复现串动机线。**编排由主体的情绪曲线驱动**(从素材提炼的 valence-over-time 一条线，§5)**，并由那里定下的弧光形状决定走法**：每个节点映一首，再据此塑曲线。**若是经典弧光**：专辑 sequencing 研究发现速度呈倒 U 型、valence/arousal 反向，故把代表作放情绪高潮(中后位)、其后留 1–2 首下行/释放。**若是渐进式、螺旋式或碎片式弧光，这个倒 U 型不是目标**——按素材真实的曲线去塑速度与动态(比如缓慢单调地转暖、没有单一峰值的轻微起伏、或一组并列的侧面而非向前推进)；代表作依然锚定在素材真实情感最强处，不论那是不是"高潮"。不论哪种形状，器乐间奏都照样作呼吸、`[End]`照样收束、总时长照样随曲目数浮动(§17)。工整vs写意基调(§13.7)：先定工整基调，写意余地0–2首由创作者把控。视角变化制造呼吸。
...
*v4.0 精简版 · 词曲声制作+三大原则+要素词汇库+AI最佳实践+提示词框架+封面设计 · 配套 song_examples*
