<!-- Ver 2026-06-16 12:40, by Claude Opus 4.8 -->
...
- **具体调号（C/A minor…）**：影响小且 Suno 不稳定，属 Tier 3。
- **常用和弦进行**（Suno 响应弱，Udio 较强，属 Tier 3 锦上添花）：
  | 流派 | 进行 |
  |---|---|
...
- **Tier 2 决定质感**：BPM、拍号、核心配器、曲式/动态弧、时代、制作风格。
- **Tier 3 锦上添花（AI 响应不稳）**：具体调号、和弦进行、旋律线、环境音、特殊效果。
...
**公式**：`[流派][子流派], [速度/能量], [核心乐器], [人声质感], [制作], [情绪]`
**数量铁律**：**4–7 个描述符**最佳。<4 太泛（十次生成各不相干）；>7 互相竞争产出"糊"（如"1960s Detroit"撞"145 BPM"、"reverb-heavy"撞"lo-fi"）。**少而精 > 多而杂。**
- 该放：流派/子流派、速度（BPM 仅近似）、乐器、人声质感、制作（lo-fi/polished/raw/warm analog）、情绪、时代。
- 无效/别放：具体艺人名（不可靠+被过滤，改用"时代+流派+制作+人声质感"替代）、专业混音术语、精确 BPM、否定式（"no drums"用 Exclude 字段，见 §23）。
...
- **像歌词单，不像段落**：清晰分段、段间空行、简单标点（逗号/破折号塑形；重标点会乱了 cadence）。
...
**④Style（6 描述符）**：
```
Mandarin emotional ballad, slow and warm, piano strings and erhu, warm aged mezzo vocals, warm analog production, deeply nostalgic
```
...
*AI 音乐分析与创作综合指南 v3.4 · 词曲分析与创作 + 要素关系 + AI 最佳实践 + 提示词框架*
*2026-06-16 · by Claude Opus 4.8*
*配套：song_examples_v3.2（示范专辑《一针一线》）、Amber-Ark 商业计划书 v1.1*
*取代：v3.1/v3.2/v3.3（其内容已整合重组入本版）*
