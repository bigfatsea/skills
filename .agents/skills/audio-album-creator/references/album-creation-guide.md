<!-- Ver 2026-07-02 01:14, by Claude Sonnet 5 -->
...
> **Track count is a beats budget, diagnosed from the material — not a slot count decided first and filled after**: 6–8 tracks is the recommended default (lean 6 for a short/single-facet story, 8 for a long/eventful one); 5 or 9–10 stay acceptable when the material's own beat count clearly calls for it, but drifting further needs a reason. What matters more than the number is the **arc's shape**, which must be diagnosed from the material before a track count or curve is chosen — it is a toolbox, not a checklist, and none of these is the default template:
> - **Classic rising-action arc**: setup → rising tension → climax → falling action → resolution. Fits material with a clear external conflict or turning point.
> - **Spiral / introspective arc**: every track circles the same core wound from a different facet — depth over progression, not a forward-moving plot.
> - **Gradual, no-climax arc**: an unremarkable life that just quietly accumulates — no inciting incident, no external conflict, no turning point to dramatize. If there's a "peak" at all, it's only where emotional density or warmth runs highest, not a plot event. Forcing a climax onto this shape produces a false, hollow high point.
> - **Fragmented / poetic collage**: deliberate ellipsis and non-chronological cuts, inviting the listener to piece the story together themselves.
> Borrowing a recognized narrative theory (three-act structure, Freytag's pyramid, 起承转合, or another) is welcome wherever it genuinely fits the material — one more tool in the box, never a mandatory template. **Never manufacture a climax, an inciting incident, or a turning point the material doesn't actually have.**
...
**Meter emotion**: 4/4 certainty · 3/4 circling · 6/8 lullaby · 2/4 brisk. **Sweet-spot runtime 2:30–3:30 per track**; total runtime scales with track count (an 8-track album ≈ 26–30 minutes; a 6-track one ≈ 20–22).
...
**Coherence comes from the invariant binder (Persona + thematic motif + unified key), not from a single uniform genre** (§24) — so genre/era-color may vary track to track by narrative beat. Thematic motif: the signature track's melody / a line of its lyric varied in another track or recurring in the instrumental; recurring motif word threads the motif line. **Sequencing is driven by the subject's emotional curve** (the valence-over-time line distilled from the material, §5) **and by the arc's shape decided there**: map each beat to a track, then shape the curve accordingly. **For a classic rising-action arc**, research on album sequencing finds tempo follows an inverted-U while valence/arousal run opposite, so place the signature track at the emotional climax (center-back), leave 1–2 tracks of descent/release after it. **For a gradual, spiral, or fragmented arc, this inverted-U is not the target** — shape tempo/dynamics to whatever the material's real curve is (e.g. a slow monotonic warming, a gentle undulation with no single peak, or a set of parallel facets with no forward motion); the signature track still anchors wherever the material's strongest moment actually falls, climax or not. Whatever the shape, an instrumental interlude still serves as a breath, `[End]` still closes it, and total runtime still scales with track count (§17). The tidy-vs-free baseline (§13.7): set the tidy baseline first, free-verse latitude 0–2 tracks at the creator's discretion. Use perspective changes to create breath.
