...
8. **禁止编造。** 报告中的命令必须出自 references；references 的每条命令必须过 `_template.zh.md` 的防幻觉自检清单。
...
写到 `/tmp/dev-env-audit-summary.json`。schema 详见 `scripts/render-card.schema.zh.md`，**必填字段**：
...
| 文件 | 何时读 |
|---|---|
| `references/python.zh.md` | Phase 1 发现 python3 / uv / pyenv / conda 任一 |
| `references/node.zh.md` | 发现 node / fnm / nvm / volta / pnpm 任一 |
| `references/java.zh.md` | 发现 java / sdkman / jenv / gradle / maven 任一 |
| `references/go.zh.md` | 发现 go / gvm / asdf-golang 任一 |
...
| `references/zig.zh.md` | 发现 zig / zigup 任一 |
| `references/julia.zh.md` | 发现 julia / juliaup 任一 |
| `references/dart.zh.md` | 发现 dart / flutter / fvm 任一 |
| `references/erlang-elixir.zh.md` | 发现 erl / elixir / mix 任一 |
| `references/cpp.zh.md` | 发现 gcc / clang / cmake / vcpkg 任一——但 clang 几乎所有 Mac 都有（Xcode CLT 自带），只在用户明确写 C/C++、或 Phase 1 发现多个 gcc 版本/vcpkg 时才深挖，不要对每台机器都跑一遍 |
| `references/_template.zh.md` | 新增或修改任何 reference 之前必读（含防幻觉自检清单），审计时不读 |
...
3. Node 是明确例外：基线首选 fnm（见 node.zh.md），不因"已有 mise/asdf"而改推。
