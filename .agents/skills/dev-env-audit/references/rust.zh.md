...
| CARGO_HOME 已设且 uv 也装在 $CARGO_HOME/bin | INFO | 不是错误，但要让用户知道 uv 装哪了（联动坑见 python.zh.md §5） |
...
- **CARGO_HOME 影响的不只是 Rust**：uv 官方安装脚本探测到 `CARGO_HOME` 会把 uv 装进 `$CARGO_HOME/bin`（详见 python.zh.md §5）。审计时发现 CARGO_HOME 已设，要顺带检查 `which -a uv`。
